...
func callProcess(caller *sap_api_caller.SAPAPICaller, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	material, batchIdentifyingPlant, batch := extractData(msg.Data())
	accepter := getAccepter(msg.Data())
	caller.AsyncGetBatchMasterRecord(material, batchIdentifyingPlant, batch, accepter)
	return nil
}

func extractData(data map[string]interface{}) (material, batchIdentifyingPlant, batch string) {
	sdc := sap_api_input_reader.ConvertToSDC(data)
	material = sdc.Batch.Material
	batchIdentifyingPlant = sdc.Batch.BatchIdentifyingPlant
	batch = sdc.Batch.Batch
	return
}
